...
// Mons are drawn as type-coloured placeholders (swap for real battle sprites later).
const TYPE_COLOR = {
  fire: 0xf08030, water: 0x6890f0, grass: 0x78c850,
  electric: 0xf8d030, normal: 0xa8a878,
};
...
class BattleScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.foe = data.foe;      // { name, mon:{name,type,dex,gen,hp,maxhp} }
    this.busy = false;
    this.ended = false;
    ...
    this.msg = this.add.text(12, H * 0.64 + 6, intro,
  ...
  makeInfoBox(mon, x, y, isYou) {
    ...
    this.add.text(x + 8, y + 5, `${mon.name}`, { fontFamily: 'monospace', fontSize: 11,
      color: '#1b2233', fontStyle: 'bold' });
    this.add.text(x + w - 8, y + 5, `${mon.type}`, { fontFamily: 'monospace', fontSize: 9,
      color: '#3b5ca8' }).setOrigin(1, 0);
    // hp bar
    this.add.rectangle(x + 8, y + 22, w - 16, 6, 0x555555).setOrigin(0);
    const fill = this.add.rectangle(x + 8, y + 22, w - 16, 6, 0x7ee787).setOrigin(0);
    const box = { fill, w: w - 16, max: mon.maxhp, hp: mon.hp,
                  txt: isYou ? this.add.text(x + w - 8, y + 20, `${mon.hp}/${mon.maxhp}`,
                    { fontFamily: 'monospace', fontSize: 8, color: '#1b2233' }).setOrigin(1, 0) : null };
  ...
  buildMoves() {
    ...
    moves.forEach((mv, i) => {
      ...
      this.add.text(bx + bw - 8, by + 7, mv.type, { fontFamily: 'monospace', fontSize: 8,
  ...
  playTurn(m) {
    ...
    const lines = m.log.map((e) => {
      const effTxt = e.eff > 1 ? " It's super effective!" : e.eff < 1 ? " It's not very effective…" : '';
  ...
  finish(m) {
    ...
    this.msg.setText(banner);
    this.add.rectangle(W / 2, H / 2, W * 0.7, 50, 0x0e1220, 0.9).setStrokeStyle(3,
      m.win ? 0x7ee787 : 0xff7b72);
    this.add.text(W / 2, H / 2, banner, { fontFamily: 'monospace', fontSize: 14,
      color: m.win ? '#7ee787' : '#ff7b72', fontStyle: 'bold' }).setOrigin(0.5);
